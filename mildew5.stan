...
data {
   ...
   int<lower=0,upper=1> Suscept_Binary[N];
   int<lower=0,upper=1> Poor_Prune[N];
   int<lower=0,upper=1> Mech_Prune[N];
 }
...
transformed parameters  {
 ...
 for (i in 1:N) {
   odds[i] = exp(beta[1] + beta[2]*Second_Year[i] + beta[3]*Suscept_Binary[i] + beta[4]*Poor_Prune[i] + beta[5]*Mech_Prune[i]);
